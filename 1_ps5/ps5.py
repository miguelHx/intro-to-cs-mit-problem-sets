...
def filter(pixels_list, color):
    ...
    """
    rgb_pixel_vector = make_matrix(color)
    new_pixels_list = []
    for px in pixels_list:
        transformed = matrix_multiply(px, rgb_pixel_vector)
        new_px = (int(transformed[0]), int(transformed[1]), int(transformed[2]))
        new_pixels_list.append(new_px)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    """
    def extract_end_bits_val(p_val):
        p = p_val
        output = 0
    ...
    if type(pixel) == int:
        return extract_end_bits_val(pixel)
    if type(pixel) == tuple:
        return (
            extract_end_bits_val(pixel[0]),
            extract_end_bits_val(pixel[1]),
            extract_end_bits_val(pixel[2])
            )
...
def reveal_bw_image(filename):
    ...
    """
    img = Image.open(filename)
    img_pixels = img_to_pix(filename)
    img_size = img.size
...
def reveal_color_image(filename):
    ...
    new_reveal_color_pxs = []
    
    def scale_pixel(p, three_lsbs):
        scaled = p
        # zero out 3 msb
        for i in range(NUM_BITS-1, NUM_BITS-4, -1):
            if scaled & (1 << i):
                scaled -= 2 ** i

        # zero out last three bits from px
        # while also moving lsb to msb
        for i in range(3):
            lsb = p % 2
            if lsb:
                scaled -= 2 ** i
                scaled += 2 ** (NUM_BITS - 3 + i)
            p = p // 2
    ...
    for px in img_pixels:
        # extract last 3 bits
        three_lsbs = extract_end_bits(3, px)
        
        scaled = (scale_pixel(px[0], three_lsbs[0]),
                  scale_pixel(px[1], three_lsbs[1]),
                  scale_pixel(px[2], three_lsbs[2]))

        new_reveal_color_pxs.append(scaled)

    return pix_to_img(new_reveal_color_pxs, img_size, 'RGB')
